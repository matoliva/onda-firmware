...
TEST_CASE("recording names use UTC paths only for reliable clocks", "[recording_naming]")
{
    char path[RECORDING_NAMING_PATH_MAX];
    const time_t timestamp = 1724545125; /* 2024-08-24T18:58:45Z */

    TEST_ASSERT_TRUE(recording_naming_time_is_reliable(timestamp));
    TEST_ASSERT_EQUAL(ESP_OK,
                      recording_naming_make_path(timestamp, 0U, path, sizeof(path)));
    TEST_ASSERT_EQUAL_STRING("/sdcard/recordings/2024-08-25/00-18-45.wav", path);
    TEST_ASSERT_EQUAL(ESP_OK,
                      recording_naming_make_path(timestamp, 1U, path, sizeof(path)));
    TEST_ASSERT_EQUAL_STRING("/sdcard/recordings/2024-08-25/00-18-45-0001.wav", path);
